...
{
    /// <summary>
    /// Kolejka priorytetowa, której priorytetem jest maksymalne q
    /// </summary>
    public class MaxPriorityQueue
    {
        ...
        {
            if (size == 0) Console.WriteLine("MaxQueue: RemoveAt : Pusta kolejka");
        ...
        {
            int index = size - 1;
            while (HasParent(index) && Parent(index).q < items[index].q)
            {
        ...
        private void HeapifyDown()
        {
            ...
            while (HasLeftChild(index))
            {
                ...
                if (HasRightChild(index) && RightChild((index)).q > LeftChild(index).q)
                {
                    smallerChildIndex = getRightChildIndex(index);
                }

                if (items[index].q > items[smallerChildIndex].q)
                {
                    break;
                }
                else
        ...
        {
            if (size == 0)
                Console.WriteLine("MaxQueue: Peek: Pusta kolejka");
